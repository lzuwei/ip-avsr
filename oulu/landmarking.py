...
if __name__ == '__main__':
    print('Generating Landmarks for OULU Dataset...')
    # use a file list instead to control which files to process
    # process only the frontal faces v1
    files = glob.glob(os.path.join(OULU_DIR, 'orig', 's[0-9]*_v1_u[0-9]*.mp4'))
    files.sort()
    ...
    for i, video in enumerate(files):
        ...
        landmarkfile = os.path.splitext(basename)[0] + '.csv'
        process_video(video,
                      os.path.join(OULU_DIR, 'landmarks', landmarkfile))
    print('All Done!')
