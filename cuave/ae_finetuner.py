...
def main():
    ...
    load_autoencoder = config.getboolean('training', 'load_ae')
    train_params = dict()
    ...
    train_data = data['trData'].astype('float32')
    test_data = data['testData'].astype('float32')
    data_matrix_len = train_data.shape[0]
    tr_vid_len = data['trVideoLengthVec']

    data_matrix = normalize_input(train_data)
    ...
    if load_autoencoder:
        print('loading autoencoder: {}'.format(ae_finetuned))
        ae = load_ae(ae_pretrained, train_params)
        ae.initialize()
