...
def main():
    ...
    stream2 = config.get('models', 'stream2')
    stream1_dim = config.getint('models', 'stream1_dim')
    stream2_dim = config.getint('models', 'stream2_dim')
    stream1_shapes = config.get('models', 'stream1_shapes')
    stream2_shapes = config.get('models', 'stream2_shapes')
    stream1_nonlinearities = config.get('models', 'stream1_nonlinearities')
    stream2_nonlinearities = config.get('models', 'stream2_nonlinearities')
    ...
    ae2 = load_decoder(stream2, stream2_shapes, stream2_nonlinearities)
    ...
    network, l_fuse = adenet_v2_2.create_model(ae1, ae2, (None, None, stream1_dim), inputs1,
                                               (None, None), mask,
                                               (None, None, stream2_dim), inputs2,
    ...
    train = theano.function(
        [inputs1, targets, mask, inputs2, window],
        cost, updates=updates, allow_input_downcast=True)
    compute_train_cost = theano.function([inputs1, targets, mask, inputs2, window],
                                         cost, allow_input_downcast=True)

    test_predictions = las.layers.get_output(network, deterministic=True)
    test_cost = temporal_softmax_loss(test_predictions, targets, mask)
    compute_test_cost = theano.function(
        [inputs1, targets, mask, inputs2, window], test_cost, allow_input_downcast=True)

    val_fn = theano.function([inputs1, mask, inputs2, window], test_predictions, allow_input_downcast=True)
