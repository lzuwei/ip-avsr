...
GRID_DIR = '/Volumes/Alienware 5/Thesis/Grid/'
...
if __name__ == '__main__':
    print('Generating Landmarks for GRID Dataset...')
    fit_image.detect = load_dlib_frontal_face_detector()
    fit_image.fitter = DlibWrapper(FACE_MODEL_PATH)
    videofiles = find_all_videos(os.path.join(GRID_DIR, 'VideoHighDef'), relpath=True)
    videofiles.sort()
    print('Found {} videos...'.format(len(videofiles)))
    for video in videofiles[31001:31010]:
        landmarkfile = os.path.splitext(video)[0] + '.csv'
        process_video(os.path.join(GRID_DIR, 'VideoHighDef', video),
                      os.path.join(GRID_DIR, 'landmarks', landmarkfile))
    print('All Done!')
